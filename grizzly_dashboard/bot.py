...
async def handle_new_activation_link(app_bot, raw_link: str) -> str:
    ...
    if "collected_links" not in cfg:
        cfg["collected_links"] = []
    if "history_since_last_collect" not in cfg:
        cfg["history_since_last_collect"] = []
        
    cfg["collected_links"].append(cleaned)
    cfg["history_since_last_collect"].append(cleaned)
...
async def callback_query_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Collect Previous ──────────────────────────────────────────────────────
        ...
        history = cfg.get("history_since_last_collect", [])
        if not history:
        ...
        try:
            ...
            logger.info(f"Sending {len(history)} collected previous links to owner...")
            with open(file_path, "rb") as doc:
                await context.bot.send_document(
                    chat_id=OWNER_ID,
                    document=doc,
                    filename="collected_previous.txt",
                    caption=f"📋 Compiled previous links ({len(history)} links since last collection)."
                )
